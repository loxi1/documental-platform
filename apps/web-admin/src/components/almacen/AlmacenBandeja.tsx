...
function guiaRecord(item: RevisionContableItem) {
  return nestedRecord(filaFactura(item), "guia");
}

function notaIngresoRecord(item: RevisionContableItem) {
  return nestedRecord(filaFactura(item), "notaIngreso", "nota_ingreso");
}
...
function documentoIdentidad(record: UnknownRecord | null) {
  if (!record) return "—";

  const serie = text(
    nestedValue(record, "serie", "serieDocumento", "serie_documento"),
    "",
  );
  const numero = text(
    nestedValue(record, "numero", "numeroDocumento", "numero_documento"),
    "",
  );
...
function searchText(item: RevisionContableItem) {
  return [
    ...
    documentoIdentidad(notaIngresoRecord(item)),
  ]
    .filter(Boolean)
    .join(" ");
}
...
function DocumentoCanonicoCell({
  record,
  label,
}: {
  ...
  const identidad = documentoIdentidad(record);
  const existe = Boolean(record);

  return (
    <span
      className={existe ? "font-medium" : "text-muted-foreground"}
      title={
        existe
          ? `${label} canónico disponible`
          : `${label} no disponible o multiplicidad no resoluble`
      }
    >
      {identidad}
    </span>
...
function ActionsCell({ item }: { item: RevisionContableItem }) {
  ...
  const suffix = tieneGrupo
    ? `?grupoFacturaId=${encodeURIComponent(String(grupoId))}`
    : "";

  return (
    <div className="flex justify-end gap-2">
      <Button asChild size="sm" variant="outline">
        <Link href={`/almacen/${expId}/ver${suffix}`}>
      ...
      {tieneGrupo ? (
        <Button asChild size="sm">
          <Link
            href={`/almacen/${expId}/editar?grupoFacturaId=${encodeURIComponent(
              String(grupoId),
            )}#adjuntar-guia-ni`}
          >
        ...
        <Button
          ...
          size="sm"
          disabled
          title="La factura no tiene grupo persistido para adjuntar Guía o Nota de ingreso"
        >
          <FilePlus2 className="h-4 w-4" />
          Adjuntar
        </Button>
    ...
    <main className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Bandeja por factura para consultar recepción y adjuntar Guía o Nota de ingreso.
        </p>
      </div>
      ...
      {error ? (
        <Card>
          <CardContent className="py-4 text-sm text-red-600">
            No se pudo cargar la bandeja de Almacén desde la fuente común por
            factura.
          </CardContent>
        </Card>
      ) : null}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-muted/40 text-left align-bottom">
                      ...
                      <th className="min-w-[115px] px-3 py-2.5">Guía</th>
                    ...
                    {pageRows.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5">
                            <DocumentoCanonicoCell
                              record={guiaRecord(item)}
                              label="Guía"
                            />
                          </td>
